...
- [x] ICAD抽出管理と内部連携データ確認をシステム設定の管理メニューへ統合し、5173から両画面へ遷移可能にする
...
最終検証: Django 65件、フロント58件、C# 14件が成功。Django system check・migration差分なし、フロント本番ビルド成功。実ブラウザではICAD登録→専用レビュー→図面管理へ戻る、製品・装置・ユニット一覧→詳細、部品一覧→詳細、システム設定の実API表示、システム設定→ICAD抽出管理／内部連携データ確認の遷移を確認し、アプリ由来のブラウザエラー0件。
