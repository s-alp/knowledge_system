...
- [x] ICAD 3D構成にノードID、親ID、深さ、子数、構成種別を追加
- [x] 1 ICDを1件として「製品・装置・ユニット」または「部品」へ振り分け、内部パーツツリーは詳細属性と判定根拠に限定
- [x] 図面管理にICAD登録、抽出、再抽出、手直し、ジョブ状態自動更新、候補確定/要手直しを統合
- [x] システム設定を実設定APIへ接続し、Gemini APIキーの値を画面/APIへ出さない
- [x] ICAD抽出管理と創屋連携データ確認をシステム設定の管理メニューへ統合し、5173から両画面へ遷移可能にする
- [x] 共有39件を全件3D snapshotとしてローカルDBへ取込
- [x] 共有39件をクリーンなSXNETセッションで最新2D再抽出し、内容あり31件・2D要素なし8件、未解決0件を台帳化
- [x] 2D全39件をローカルDBへ再取込し、全件で2D/3D snapshotを保持
- [x] 自動起動したICADの終了処理から強制終了を廃止し、保存確認だけ「いいえ」を選ぶ安全終了を実機確認
- [x] 全39件版fixtureと創屋向けAPI・責務資料を最終生成し、実ブラウザ通し検証を完了

最終検証: Django 65件、フロント58件、C# 14件が成功。Django system check・migration差分なし、フロント本番ビルド成功。実ブラウザではICAD登録→専用レビュー→図面管理へ戻る、製品・装置・ユニット一覧→詳細、部品一覧→詳細、システム設定の実API表示、システム設定→ICAD抽出管理／創屋連携データ確認の遷移を確認し、アプリ由来のブラウザエラー0件。

判定基準: 創屋本番DBには登録・変更・削除を行わず、読み取り確認とローカル成果物だけを扱う。
...
- [x] タグ自動取得の配置方針を画面へ反映。フロントの `システム設定` に `タグ自動取得設定` を追加し、設定値、Gemini低温度JSON分類、対象別マッピング、2D/3D採用ルールを表示。Djangoローカル確認画面にも `/drawing-metadata/system/tag-automation/` と左メニューの `システム設定` リンクを追加し、図面管理側のタグ候補レビュー/再抽出/手動補正が運用導線であることを明記
