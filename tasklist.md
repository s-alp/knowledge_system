...
## 創屋向け説明資料の非技術者対応（2026-08-05）

- [x] 創屋から「渡した資料がエンジニア向けで説明に使えない」との指摘を受け、既存技術文書の書き換えではなく非技術者向け資料の追加で対応する方針を決定
- [x] `handoff/souya_tag_extraction/recipient_docs/docs/overview_for_users.md` を新規作成。コマンド・JSON・ソース構成を使わず、目的、導入前後の違い、取り出せる情報、対象外範囲、必要な環境、想定質問、技術用語との対応表で構成
- [x] 配布PDFを読み手別に2種類へ分離。`概要ガイド`は概要文書のみ、`利用ガイド`は概要を先頭に置きREADMEと4技術文書を継続収録
- [x] `generate_souya_recipient_guide_pdf.py` を収録範囲・表紙・ヘッダー・メタデータの設定単位へ整理し、`--document-set` で両方を生成できるようにした
- [x] `build_souya_tag_extraction_package.py` の受領者向け文書許可リストへ概要文書を追加し、2PDFを同梱するようにした
- [x] `prepare_souya_tag_extraction_handoff.py` で2PDFを生成し、片方だけ生成して次回実行が止まらないよう出力先を事前検査するようにした
- [x] `verify_souya_tag_extraction_handoff.py` を2PDF検証へ更新し、タイトルメタデータで概要・利用の取り違えを検出するようにした
- [x] READMEの資料選択表と担当別表へ概要資料の行を追加し、冒頭で非技術者の参照先を明示
- [x] AGENTS.md、文書索引、最小パッケージ仕様を2PDF構成へ更新
- [x] 文書監査 `audit_tag_documentation.py` エラー0、外部共有監査 違反0、PDF生成で概要6ページ・利用28ページ、空白ページなし、しおりあり、タイトルメタデータ一致を確認
- [ ] Windows側でPDF全ページを画像化して文字切れ・重なりを目視確認する
- [ ] 創屋が指している資料が引き渡しパッケージの説明文書で相違ないか、想定読者が創屋社内か客先かを確認する
