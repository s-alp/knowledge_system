...
- [x] アセンブリ/サブアセンブリを「製品・装置・ユニット」、末端ノードを「部品」として実データ一覧・詳細へ表示
...
最終検証: Django 65件、フロント58件、C# 14件が成功。Django system check・migration差分なし、フロント本番ビルド成功。実ブラウザではICAD登録→専用レビュー→図面管理へ戻る、製品・装置・ユニット一覧→詳細、部品一覧→詳細、システム設定の実API表示を確認し、アプリ由来のブラウザエラー0件。
