"""test_apiの正常系・境界値・失敗時の挙動が変わらないことを自動テストで確認する。

テスト名は利用者から見た前提と期待結果を表し、失敗時は対象実装の契約違反を示す。
外部API・時刻・ファイル操作はfixtureやmockへ置き換え、再現可能性を保つ。
主な検証観点:
- 図面登録から2D/3D抽出ジョブ起票までのHTTP契約。
- snapshot、合成結果、タグ根拠を返す詳細APIの項目。
- 手動補正とレビュー判断が再取得後も保持されること。
- ビューワーbootstrapと対象物APIが同じ図面範囲を使うこと。
- 不正ID、重複ジョブ、危険なパスを明示的なエラーにすること。
"""
import tempfile
import json
