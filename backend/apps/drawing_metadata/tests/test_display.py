"""test_displayの正常系・境界値・失敗時の挙動が変わらないことを自動テストで確認する。

テスト名は利用者から見た前提と期待結果を表し、失敗時は対象実装の契約違反を示す。
外部API・時刻・ファイル操作はfixtureやmockへ置き換え、再現可能性を保つ。
主な検証観点:
- 未抽出・0・空文字を画面上で混同しないこと。
- 2Dのビュー、レイヤー、印刷枠集計がraw件数と一致すること。
- 3Dの質量、材質、パーツ付加情報を根拠付きで表示すること。
- 長い値を省略しても、確認用の元データを失わないこと。
- ビューワー、タグレビュー、引継ぎ画面で表示契約が一致すること。
"""
import json
