...
## 創屋向け配布コードのファイル規模
- 人が変更箇所と影響範囲を追えることを優先し、1ファイル1,000行以上は責務分割を検討する警戒線とする。
- 行数だけを減らす分割は行わず、公開入口、処理順序、2D、3D、材質、文字列、規則、外部I/Oなど、変更理由が異なる責務で分ける。
- 1,000行以上を残す場合は、処理の凝集度、分割時の循環依存、生成コードかどうかを確認し、残す理由と読む順序を配布文書へ記載する。
- 配布コードを分割した場合は公開APIを維持し、既存の2D・3D結果が変わらないテストを実行する。
