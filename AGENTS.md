...
- C. 図枠のラベルと値が別テキスト要素になっている実例の収集(座標ペアリング実装の要否と距離パラメータ決め)。
...
- H. 客先・装置・案件の辞書語彙と、フォルダ命名規則(`日付_案件名(担当者様)`)の一覧化。材質偏重タグの解消はここが効き筋。
- 2026-07-17 実装済み(Windows側は `git pull` 後に `python manage.py re_normalize_snapshots` で既存データへ反映):
