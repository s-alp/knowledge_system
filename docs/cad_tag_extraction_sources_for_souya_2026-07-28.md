...
| 規格 | 6 | JIS、ISO、DIN、ANSI、幾何公差、溶接記号 |
...
客先辞書と案件辞書は初期エントリ0件で、創屋様のDBまたは辞書JSONから注入する運用値を正本とします。照合対象はファイルパスのトークンだけでなく、モデル情報・図面内文字・DXFレイヤー等を合成した検索トークン列（`part_keywords`）です。実運用の客先名・案件名・別名は、共有範囲の承認後に登録してください。
