...
- 2D対象: 38ファイル（共有39件中、1件は現端末から原本未アクセス）
- ビュー/用紙数: 208
- 印刷枠数: 32
- レイヤー数: 9,690
- 取得対象要素: 30,105件
...
2026-07-17 に共有manifestのうち現端末からアクセスできる2D対象38件を最新runnerで再抽出し、旧coverage JSONを置き換えた。
...
- 2D対象: 38ファイル（共有39件中1件は現端末から原本未アクセス）
- ビュー/用紙数: 208
- 印刷枠数: 32
- レイヤー数: 9,690
- 取得対象要素: 30,105件
...
旧manifestでは `ビュー情報なし17ファイル`、`印刷枠情報なし17ファイル`、`レイヤー情報なし17ファイル` だったため、最新抽出器で全ビュー・印刷枠・レイヤー取得は大きく改善した。現時点で再抽出できていない1件は `J:\不二越5\260703_次期円筒研削盤開発(竹中様)\作業\18T5-10BF(8).icd` で、現端末から `file_not_found` となる。再抽出条件は、当該共有パスを接続し直すか、アクセス可能な移動後パスを特定してmanifest/ローカルDBを明示的に付け替えること。
...
- 最新3D抽出利用可能: 39/39
- 共有元ファイル欠落: 1件（`J:\不二越5\260703_次期円筒研削盤開発(竹中様)\作業\18T5-10BF(8).icd`）
- 未解決: 0件（欠落1件は再抽出条件を記録済み）

「2D要素なし」8件は抽出失敗ではない。VS/ビューとレイヤー情報は取得できた一方、文字・寸法・図形primitive・印刷枠が0件だったため、内容を捏造せずその状態を記録している。

アクセス可能38件の2Dカバレッジは `output\souya_handoff\icad_2d_extraction_coverage_all_shared_2026-07-15.json` に保存した。全ビュー208、印刷枠32、レイヤー9,690、検査可能要素30,105、文字1,752、寸法2,404、図形primitive 25,903を取得した。要素のビュー未所属は0件、印刷枠内5,088、枠外23,849、判定不明1,168である。判定不明とレイヤー未所属は証拠として保持し、自動タグ採用では印刷枠内を優先する。

当初の参照先で見つからなかった `36555211A01.icd` と `32791729A01.icd` は、同じ共有案件内の移動後パスを特定した。manifestとローカルDBの保存パスを更新済みである。現時点で原本未アクセスとして残るのは `18T5-10BF(8).icd` の1件で、再抽出条件は共有パスの再接続またはアクセス可能な移動後パスの特定である。
