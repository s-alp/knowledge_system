...
| 連携仕様集計 | `GET /api/v1/drawing-metadata/handoff-summary` | システム設定内の `API仕様・連携仕様` で使う登録ICAD、対象別payload、APIリンクの集計を返す |
...
- 2026-07-16 DB監査: snapshot 84件、タグ128件、禁止タグ0件、取得元欠落0件、根拠欠落0件、信頼度欠落0件、採用理由欠落0件。
...
2026-07-16に共有抽出から10ファイルを再評価した。図枠候補32件のうち、25件は値なし/値として使えない候補としてGemini送信対象外にした。実APIへ送ったのは値あり4候補だけで、温度は `0.0`、用途は `title_block_candidate_field_classification_only` として記録した。プロンプトと事前フィルタで以下を固定している。
...
誤採用防止は確認できたが、ルール抽出を上回る新規採用値は0件である。したがってGeminiは任意補助のままとし、品質向上効果があるとは断定しない。検証は `scripts/probe_title_block_llm.py` と `scripts/evaluate_title_block_llm_probe.py` で再実行でき、Gemini APIエラー、guardrail false positive、wrong field が出た場合は評価コマンドを失敗させる。
...
- Chrome実操作: 製品・部品詳細、取得根拠、編集画面、図面紐づけ候補38件、抽出状態非表示、システム設定内の `API仕様・連携仕様` / `ICAD抽出管理` 表示、旧Django画面や図面管理へ遷移しないことを確認
