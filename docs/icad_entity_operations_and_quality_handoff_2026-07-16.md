...
- `subassembly` は親子の外部参照関係または手動確定が得られた場合だけ扱う。自動で捏造しない。
...
| 図面候補 | `GET /api/v1/drawing-options?q=...&limit=100` | 図面名・保存先を検索する。実ブラウザで67件表示を確認 |
...
- Chrome実操作: 製品・部品詳細、取得根拠、編集画面、図面紐づけ候補67件、抽出状態非表示、システム設定から旧Django画面へ遷移しないことを確認
