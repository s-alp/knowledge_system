...
> **文書状態: 履歴資料（再設計の判断記録）**
> 3層設計、根拠保持、2D/3D分離、タグの限定採用は現行コードへ反映済みである。
> 現在の実装仕様は [`tag_extraction_and_assignment_current_spec_2026-07-29.md`](tag_extraction_and_assignment_current_spec_2026-07-29.md) を正とする。
...
- raw抽出、canonical属性、derived tagsの3層: 実装済み
- 2D/3D別snapshotと照合結果: 実装済み
- 根拠、信頼度、採用理由、競合: 実装済み
- Gemini分類は当時実装済みだったが、2026-07-29に外部AIの実行経路・設定・専用スクリプトを削除
- 辞書DB、手動補正、レビューUI: 実装済み
- 創屋本番DB/APIへの保存: 未接続

本文中のGemini案、v2案、実装順序、完了チェックリストは当時の設計記録として読む。
...
| 重量 / 体積 / 質量属性 | `SxWF.getExtent()`、`SxWF.getEntList()`、`SxEnt.getMass()` | 実サンプル3件で取得確認済み。部品単位/グループ単位の粒度と例外条件は継続確認 |
...
2026-07-14 追加実装では、Django正規化層に `title_block_candidates` と `title_block_fields` を追加した。図番、図面名、材質、重量、表面処理、塗装指示、尺度、設計者、検図者、承認者、日付、改訂、PRFX、ユニット番号を初期辞書で候補化する。採用値はCAD内の同一文字または次行に存在するものだけで、印刷枠外文字、ラベル単体、長すぎる注記は採用しない。

同じく `geometry_feature_candidates` を追加し、ハッチング、表面粗さ、切断線、データム、幾何公差、仕上げ記号、長穴候補、穴候補を primitive 由来の特徴候補として保持する。長穴/穴は確定判定ではなく低信頼候補に止める。
...
照合結果は、全属性の根拠を残す `reconciledAttributes` と、人が採用値を見るべき `conflicts` を分ける。`source_kind`、`confidence_summary`、件数系、存在フラグ系の差分は品質診断として有用だが、設計者レビューやRAG投入前確認に混ぜるとノイズになる。そのため `diagnosticConflicts` に分離し、JSON証跡には残しつつ、自動タグ除外やレビュー警告には使わない。
...
創屋向けの図面/プロジェクト/製品・装置・ユニット/部品別連携項目表は `docs/souya_icad_tag_attribute_handoff_2026-07-14.md` に分離した。

本番移植前には、2D/3D プレビュー時と同様に、ナレッジシステム本番側の実データ構造、API 返却、画面表示項目を確認し、それを模した fixture / 検証用API をこちらで作る必要がある。
...
実画面の詳細表示では、図面詳細に `タグ` と `属性情報` が表示され、2D/3Dプレビュー切替も同一画面にある。製品・装置・ユニット詳細と部品詳細には `属性情報` は見えるが、タグ欄は見えない。プロジェクト詳細にはタグ/属性の表示口が見えない。したがってタグ活用対象は図面だけに閉じず、創屋へは以下の差分を前提に渡す。

| 対象 | 既存画面の見た目 | 連携方針 |
| --- | --- | --- |
| 図面 | `タグ` と `属性情報` が見える | タグ候補・属性候補の初期反映先 |
| プロジェクト | タグ/属性の表示口は見えない | 客先、案件、装置系列タグを載せるなら表示/API追加を相談 |
| 製品・装置・ユニット | `属性情報` が見える | ユニット番号、工程、装置カテゴリなどを属性連携候補にする |
| 部品 | `属性情報` が見える | 材質、メーカー、パーツ付加情報、参照ICADパスを属性連携候補にする |

2026-07-15 に本番詳細ページを再確認した。図面詳細、製品・装置・ユニット詳細、部品詳細、プロジェクト詳細の現在表示スクリーンショットは `output\knowledge_ui_screenshots_2026-07-15\production-*-detail-current-2026-07-15.png` に保存した。登録、変更、削除は行っていない。再確認でも、図面はタグと属性の直接表示口あり、製品・装置・ユニットと部品は属性表示口あり、プロジェクトはタグ/属性表示口なし、という判断は変わらない。

図面詳細の3D表示へ切り替えた際、`/web/public/models/test_000445.gltf` の読み込みで `Unexpected token '<'` のエラー画面が出た。読み取り確認のみで登録、更新、削除は行っていない。この表示不具合は抽出器とは別件だが、2D/3Dプレビュー fixture を作るときに実画面差分として創屋へ伝える。
...
同日の追加実装で、2D/3D照合のレビュー対象を整理した。`reconciledAttributes` は従来どおり全差分を保持し、`conflicts` は材質、重量、図番、図面名など採用値レビューが必要な属性に限定する。内部品質・件数・抽出元差分は `diagnosticConflicts` に分ける。創屋引き渡しfixture 25図面ではレビュー競合0件、診断差分93件で、ローカル詳細画面にも `2D/3Dレビュー競合数` と `2D/3D診断差分数` が表示されることを確認した。
...
- [x] 創屋との責任分界を整理した
- [x] 3D重量・質量・体積・面積・重心を実サンプルで取得確認した
- [x] 本番ナレッジシステム実画面を読み取り専用で確認し、図面/プロジェクト/製品・装置・ユニット/部品の表示口差分を整理した
- [x] 2D図枠欄名の初期辞書と候補レビュー表示を追加した
