...
完成版の表示UIは、本リポジトリの`integrations/2D_3D_CAD_VIEWR/frontend`を基準にする。利用者向け統合フロントは通常`http://127.0.0.1:5173/`である。Django HTMLは`/internal/drawing-metadata/`の内部検証画面であり、創屋へ見せる完成UIの土台にはしない。

## 2026-07-29 反映事項

- 抽出管理、2D/3D再抽出、候補タグ、手動補正、レビューをVite画面へ実装済み。
- タグ辞書CRUDを「システム設定 > タグ辞書管理」へ実装済み。
- 製品・装置・ユニット／部品の名称、図面番号、属性、タグ、根拠、照合結果を表示する。
- アセンブリ本体の材質・付加情報と、外部参照パーツの名称・材質・付加情報を別行で表示する。
- 自動タグは現行`tag_builder.py`の一覧に限定し、名称、図番、尺度、件数、未解決材質は自動タグ化しない。
...
- ICAD原文、印刷枠、ビュー、レイヤー、文字座標、明示ラベル、タグ辞書だけでは確定できない値。Geminiを含む外部AIは使用せず、曖昧な候補はraw証跡とレビュー対象に残す。
...
| 深さ1以上のノードで外部参照、参照モデル名、参照モデルパスあり | 製品・装置・ユニット | サブアセンブリ候補。子ノード数が0でも外部参照なら部品数ではなく子製品・装置・ユニット数へ数える |
| 中間ノードで外部参照なし | 製品・装置・ユニット | アセンブリ/内部構成診断 |
| 子ノードなし、かつ外部参照/参照モデルなし | 部品 | 末端部品 |
