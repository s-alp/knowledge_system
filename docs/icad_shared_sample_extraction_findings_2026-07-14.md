...
当初は `source-kind` 指定で2D/3Dを試しているだけであり、`TR1D9Q00027.icd` の2D結果や `XH3001-M08007-01.icd` の2D結果のように、空に近い抽出でも成功扱いになっていた。

現在は `detect` と抽出snapshot/diagnosticsで、ICD内の2D/3D有無、2Dセグメント数、ビュー・レイヤー・印刷枠、3Dパーツツリー・材質・重量系の取得状態を分けて記録する。未抽出や空に近い2Dは「存在しない」と断定せず、再抽出条件やデータ条件として扱う。
...
- 空に近い2Dや抽出不足は `extractionDiagnostics` と再抽出profileで追跡する
...
- 2Dジオメトリの詳細意味付けは、検索タグ化ではなく属性・レビュー候補として実装済み。`SxGeomSpline2D`、ハッチング、表面粗さ、切断線、データム、楕円/長穴候補、穴/円候補を raw primitive、`geometry_feature_candidates`、各件数属性として保持する。形状存在だけでは検索・分類タグへ採用しない。
- 2D図枠欄名辞書は初期実装済み。Gemini API低温度JSON分類も曖昧欄名の補助に限定して組み込み済みで、CADにない値を生成・採用しない。
- 3D材質 API は `SxEnt.getInfMaterialList()` / `SxEntPart.getInfMaterialList()` を実サンプルで確認済み。未解決材質コードは通常タグにせず、属性・レビュー要確認情報として保持する。
- 本番ナレッジシステム向けの創屋引き継ぎ仕様は、図面/プロジェクト/製品・装置・ユニット/部品別の連携候補、API仕様、レビューサマリfixtureとして整理済み。ただし創屋本番DBへの登録、変更、削除は行わない。
...
- `unresolved`: 捨てずに `unresolved_material_keywords` とレビュー条件に残す。検索・分類用タグにはしない
...
- `ZZZ` や `75` のように材質辞書上の意味が不明または客先固有の値がある。実装では `ZZZ`, `75`, `CDQ` を `unresolved_material_keywords` に分離し、通常の `材質:<値>` タグにも要確認タグにもせず、レビュー属性とRAG投入時の `reviewFlags` として保持する。
...
## 14. 2Dジオメトリの primitive 化

2026-07-14 以降の追加実装で、以下の2Dジオメトリを `geometry_primitives` として保持するようにした。
