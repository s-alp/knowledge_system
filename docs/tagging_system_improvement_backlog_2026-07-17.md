...
対応状況は 2026-07-17 実装反映(§8)を参照。

| # | 区分 | 改善点 | 優先度 | 対応状況 |
| --- | --- | --- | --- | --- |
| 1 | ライフサイクル | 再抽出で手動タグ・タグ削除が消える(手動補正が再適用されない) | 重大 | 対応済み |
| 2 | ライフサイクル | 削除したはずの自動タグが統合結果で復活する | 重大 | 対応済み |
| 3 | ライフサイクル | 属性の手動補正を項目ごとに保存すると、以前の補正記録が丸ごと置き換わる | 重大 | 対応済み |
| 4 | ドメインモデル | 製品・装置・ユニット・部品のエンティティが存在せず、全てが図面単位のJSON文字列 | 高 | 未対応(創屋確認事項) |
| 5 | タグモデル | タグの正体が「表示文字列(`客先:xxx`)」で、名前空間・値・辞書との対応を持たない | 高 | 一部対応 |
| 6 | 辞書 | 客先・装置・メーカー・規格辞書がPythonコード内のハードコード定数 | 高 | 対応済み |
| 7 | 正規化 | 表記ゆれ吸収が `.lower()` のみ。全角半角・カナ・NFKC正規化が無い | 高 | 対応済み |
| 8 | 正規化 | 部分文字列一致による誤ヒット(`ses` が `hoses` 等に一致)と先勝ち単一値マッチ | 高 | 対応済み |
| 9 | 正規化 | `project_name`・`equipment_name`・`module_name`・`drawing_number`・`title_block_fields` 等が一切埋まらない | 高 | 未対応(実データ検証待ち) |
| 10 | 根拠 | 「どのトークンからどのタグが付いたか」の根拠(evidence)が保存されない | 高 | 対応済み |
| 11 | 部品情報 | 部品の name/comment/tree_path の対応関係が正規化時に分解され、部品単位の情報が消える | 高 | 対応済み |
| 12 | 合成 | 2D/3D競合時の `chosenMode` 記録が実際の採用値と食い違う。競合属性はタグ自体が消える | 中 | 対応済み |
| 13 | API/UI | 一覧のフィルタ・ページネーション未実装。タグ・属性での絞り込みができない | 中 | 対応済み(PoC水準) |
| 14 | 契約 | ドキュメント(snake_case)と実装(camelCase)のスキーマ乖離、`attributeGroups` の型不整合 | 中 | 対応済み |
| 15 | 運用 | 辞書・ルール改訂後に再抽出なしで再正規化する手段(コマンド)が無い | 中 | 対応済み |
...
同日、`backend/apps/drawing_metadata` に対して以下を実装した。スキーマ変更点の詳細は
`docs/extraction_result_schema_2026-05-28.md` §12 を参照。全26テスト(既存12 + 追加14)と、
登録→抽出→補正→再抽出→統合確認のHTTP経由スモークテストで確認済み。

### 対応済み

- **#1〜#3 手動補正ライフサイクル** (`services/overrides.py` 新設)
  - 確定属性・確定タグは常に「自動抽出値 + manual_overrides」の合成として再計算する。
  - `merge_manual_overrides` で補正はキー単位マージ。`null` 指定で補正解除し自動値へ戻る。
  - タグは `自動タグ - removed + added` で計算し、再抽出・再正規化後も手動分が消えない。
  - 統合(`composition.py`)も全モードの removed/added を尊重する。
- **#5(一部) タグ構造化** (`services/tag_builder.py`)
  - タグへ `namespace` / `value` / `evidence` / `tag_rule_version` を追加。表示文字列は互換維持。
  - タグ中間テーブル化(完全一致フィルタ・改名耐性)は本体統合時の課題として残る。
- **#6 辞書DB化** (`models.TagDictionaryEntry` + `services/dictionaries.py`)
  - admin から編集可能。DB が空の kind は seed 定数へフォールバック。
  - `seed_tag_dictionaries` コマンドで初期投入。
- **#7 #8 正規化底上げ** (`services/text_matching.py` 新設)
  - NFKC + casefold で全角半角・大小文字を吸収(全角 `ＳＥＳ` が一致する)。
  - ASCII 候補は語境界一致(`ses` が `hoses` / `processes` に誤ヒットしない)。
  - 複数候補を `customer_name_candidates` 等に全件保持し、主候補は辞書優先順で決定。
    複数候補時は confidence を下げる。
- **#10 根拠保存**
  - `canonical_attributes.match_evidence` と各タグの `evidence` に
    「どのフィールドのどのトークンがどの別名に一致したか」を保存(1値あたり上限5件)。
- **#11 部品構造保持**
  - `canonical_attributes.parts` に部品単位の name/comment/tree_path/参照/フラグを構造のまま保持。
- **#12 合成の整合**
  - `chosenMode` は実際の採用元(`manual_3d`/`manual_2d`/`3d`/`2d`)を記録。
  - 競合キー由来のタグは除外せず `confidence=low` で残す。
  - `attributeGroups` から型不整合だった conflicts グループを削除(トップレベル `conflicts` に一本化)。
- **#13 API/UI(PoC水準)**
  - 一覧APIへ `customer` / `equipmentCategory` / `tag` / `mode` / `jobStatus` フィルタと
    ページネーション(envelope形式)を追加。フィルタは統合結果の保存値を参照。
  - 一覧シリアライザの N+1 を解消。`sourcePath` 重複登録を 400 で拒否。
  - HTML補正フォームの JSON / 抽出モードを検証し、不正入力を 500 にしない。
- **#14 契約整合**: スキーマ文書へ実装準拠の改訂(§12)を追記。
- **#15 再正規化**: `re_normalize_snapshots` コマンド(`--stale-only` / `--drawing-id` / `--mode`)。
  監査ログへ `re_normalize` アクションを記録。
- **付帯**: 統合結果を `DrawingComposedMetadata` として永続化(抽出成功時・補正保存時・再正規化時に更新)。
  監査ログから raw_extract 全文の二重保存を廃止(ジョブID参照に変更)。
  `extraction_status` は warnings 有りで `partial`。normalizer/tag_rule バージョンを 1.1.0 へ。

### 未対応(意図的に残したもの)

- **#4 製品・装置・ユニット・部品のエンティティ化**: 創屋側本体にマスタが存在する可能性が高く、
  勝手にマスタを二重化しない方針(§6-4)。`parts` の構造保持までを下準備として実施済み。
  §7 の確認事項を解消してから着手する。
- **#9 タイトルブロック等の未実装属性**: 2D タイトルブロックの項目対応は ICAD 実データでの
  検証が必要なため、実サンプルでの PoC(CLAUDE.md の「ICAD 2D 抽出 PoC」)と合わせて行う。
- **#5 の残り**: タグ中間テーブルによる完全一致フィルタ・タグ改名は、本体のタグ体系確認後に実施する。
