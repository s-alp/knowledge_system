...
- 図面名、部品名、製品名、装置名、ユニット名を対象別に分離し、汎用`name`を廃止した。
- 図面番号を図枠明示値、図面文字とファイル名の一致、ファイル名の順で決める。
- ICAD版差で未知の実型名になった文字要素を、非空`txt`がある場合だけ救済する。
- 印刷枠内外が全文字`unknown`の場合は、判定材料なしとして文字を名称解析へ渡す。
- 内部パーツと外部参照パーツの名称、材質、付加情報、候補を分離した。
- DXFのDIMSTYLE、寸法override、TOLERANCEを解析し、ICAD/DXF共通で`寸法あり`、`寸法公差あり`、`幾何公差あり`、溶接、硬度タグを生成する。

この章は追加差分であり、2026-07-14時点の実測値は当時のsnapshotとして本文に残す。
...
| 図枠候補の分類 | 実装済み | ICAD原文、印刷枠、ビュー、レイヤー、文字座標、明示ラベル、タグ辞書だけで分類する。2026-07-29にGemini実行経路を削除し、創屋側でも外部AIを使用しない |
