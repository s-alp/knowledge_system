...
## 5.3 一括取得後の自動抽出・自動付与フロー

一括取得 API で生データを取った後は、以下の順で処理する。

1. 一括取得
   - 3D は `getInfPartTree()` を起点に、部品階層、部品詳細、任意情報をまとめて取得する。
   - 2D は `getGeomList()` を起点に、文字、注記、寸法、公差、溶接、バルーン系をまとめて取得する。
2. 生抽出の保存
   - 取得結果をそのまま `raw_extract.payload` に保存する。
   - ここでは意味付けやタグ化をまだ行わない。
3. 項目ごとの抽出
   - 文字列群、寸法値群、部品名群、コメント群、任意情報群を型ごとに分離する。
   - 図面番号候補、案件候補、客先候補、装置カテゴリ候補などの候補値を切り出す。
4. 正規化
   - 表記ゆれ、別名、略語、全角半角差を辞書で吸収し、`canonical_attributes` に寄せる。
   - 候補が複数ある場合は優先順位ルールで 1 つに絞るか、多値属性として保持する。
5. 自動タグ生成
   - `canonical_attributes` と辞書から `derived_tags` を生成する。
   - 例:
     - `customer_name=コマツ小山` -> `客先:コマツ小山`
     - `equipment_category=ガントリー` -> `装置:ガントリー`
     - `heat_treatment_keywords` あり -> `工程:熱処理`
   - 自由語はそのままタグ化せず、辞書や属性に落ちたものだけを確定タグにする。
6. 信頼度付与
   - 項目ごとに `source`, `confidence`, `manual_flag=false` を持たせる。
   - OCR 起点や曖昧一致は低め、ICAD ネイティブの明示項目は高めにする。
7. 自動付与結果のプレビュー
   - `図面管理` の登録時プレビューで、属性・タグ・根拠断片を確認できるようにする。
   - ここで人が修正した内容は `manual_overrides` に保存する。
8. 確定保存
   - `raw_extract`, `canonical_attributes`, `derived_tags`, `manual_overrides` を分けて保存する。
   - viewer と RAG は、確定後の `canonical_attributes` と `derived_tags` を利用する。

### 自動抽出と自動付与を分ける理由

- 自動抽出:
  - 元データから候補値を取り出す工程
  - 例: 図面内の文字列、寸法値、部品名、任意情報の取得
- 自動付与:
  - 抽出結果を属性やタグとして意味付けして保存する工程
  - 例: `SMC` を `maker_keywords` へ、`ガントリー` を `equipment_category` とタグへ反映

この 2 段階を分けることで、抽出ロジックの見直しと、辞書・付与ルールの見直しを独立して改善できる。
