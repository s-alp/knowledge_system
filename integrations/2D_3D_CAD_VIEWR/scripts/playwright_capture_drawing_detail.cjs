...
const screenshotPath = path.join(runtimeDir, 'drawing-detail-page.png');
const summaryPath = path.join(runtimeDir, 'drawing-detail-summary.json');

fs.mkdirSync(runtimeDir, { recursive: true });

async function collectTextList(page, selectors, limit = 50) {
...
async function openFirstDrawing(page) {
  const rowLinkCandidates = [
    'table tbody tr td a',
    'table tbody tr td button',
    'table tbody tr',
  ];

  for (const selector of rowLinkCandidates) {
    const locator = page.locator(selector);
    const count = await locator.count();
    if (!count) {
      continue;
  ...
  throw new Error('図面詳細へ進めるクリック対象を見つけられませんでした。');
}
...
async function main() {
  ...
  try {
    ...
    const clickedSelector = await openFirstDrawing(page);
    await page.waitForLoadState('networkidle', { timeout: 30000 }).catch(() => null);
    await page.waitForTimeout(3000);

    const headings = await collectTextList(page, ['h1', 'h2', 'h3', '[role="heading"]']);
    const tabsAndButtons = await collectTextList(page, ['button', 'a', '[role="tab"]', '[role="button"]'], 80);
    const labels = await collectTextList(page, ['dt', 'th', 'label', '.MuiFormLabel-root', '.MuiTypography-root'], 100);
    const rows = await collectTextList(page, ['table tr', '[role="row"]', 'li'], 100);
