...
DEFAULT_OUTPUT = (
    ...
    / "output"
    / "pptx"
    / "20260805_ナレッジシステム_タグ属性抽出_エンドユーザー様向けご説明_r1.pptx"
)
...
def _chapter_slide(prs, number: str, title: str, heading: str, bullets: list[str]):
    ...
    _set_lines(_ph(slide, 2).text_frame, [(number, {"size": 12, "bold": True, "color": BLUE})])
    _set_lines(_ph(slide, 0).text_frame, [(title, {"size": 26, "bold": True, "color": INK})])
...
def _note(slide, y: float, text: str) -> None:
    ...
    _textbox(slide, 0.5, y, 12.33, 0.5, [(text, {"size": 9.5, "color": MUTED, "space_after": 0})])
...
def build_deck(output_path: Path) -> Path:
    ...
    summary = [
        ...
        (
            "03",
            "CADの買い足しは\n必須ではない",
            "3D CAD（ICAD）が要るのは図面を登録する時だけです。"
            "検索や閲覧にCADは要りません。CADなしの構成も選べます。",
        ),
    ]
    ...
    _note(
        slide,
        6.25,
        "本資料は、導入をご検討いただく立場の方に向けて、仕組みの位置づけと前提条件を整理したものです。"
        "操作方法や技術仕様は含みません。",
    )
    ...
    slide = _two_column_slide(
        prs,
        "フォルダ整理や台帳作成では、なぜ届かないのか",
        "人手で情報を写す方式は、増え続ける図面に追いつけません",
        "これまでの対策",
        "この仕組みの考え方",
    )
    _textbox(
        ...
        + [
            ("続かない理由", {"size": 13, "bold": True, "color": BLUE, "space_after": 9})
        ]
        + _bullet_lines(
            [
                "図面が増えるたびに、人手の作業も増える",
    ...
    _textbox(
        ...
        + [("", {"size": 10, "space_after": 10})]
        + [
            ("人が続けやすい理由", {"size": 13, "bold": True, "color": BLUE, "space_after": 9})
        ]
        + _bullet_lines(
            [
                "登録すれば手がかりが付くので、作業が増えない",
                "同じ規則で処理するため、表記がぶれない",
                "過去の図面も、まとめて読み取り直せる",
            ],
            size=12,
    ...
    steps = [
        ...
        ("STEP 3", "見出しを付ける", "登録された言葉と照らし合わせ、客先・装置・材質などの見出しを付けます。判断の根拠も一緒に残します。"),
    ]
    ...
    _textbox(
        ...
        1.1,
        [
            ("この3つは自動で動きます。設計者が入力する作業はありません。", {"size": 14, "bold": True, "color": BLUE, "space_after": 8}),
            (
                "処理は図面を登録したときに1回だけ動きます。以降の検索・閲覧では、保存された情報を見るだけです。",
    ...
    _textbox(
        ...
        [
            (
                "この状態になると、「A社向けで、SUS304を使ったガントリー」のような条件で、図面を開かずに絞り込めます。",
    ...
    slide = _content_slide(
        prs,
        "実際の設計図面での確認結果",
        "当社が実務で使われている設計図面39件を、登録から表示まで通して確認しました",
    )
    _table(
        slide,
        0.5,
        ...
        ["確認した観点", "結果", "評価"],
        [
            ["最後まで処理できたか", "39件すべて（処理できなかった図面なし）", "落ちずに通る"],
            ["部品の構成を取り出せたか", "39件すべて", "全件で取得"],
            ["質量・重量を取り出せたか", "39件中38件", "ほぼ全件で取得"],
            ["材質を取り出せたか", "39件中33件", "8割超で取得"],
            ["客先・装置の見出しが付いたか", "少数", "登録した言葉の数に依存"],
        ],
    ...
    _textbox(
        ...
        1.6,
        [
            ("客先・装置の見出しが少ない理由", {"size": 12, "bold": True, "color": BLUE, "space_after": 6}),
            (
                "この確認を行った時点では、照合に使う言葉（社名・装置名）を検証用に3件しか登録していませんでした。"
                "読み取り側の限界ではなく、実際に使う社名・装置名を登録すれば付くようになります。",
                {"size": 11, "color": INK, "space_after": 8},
            ),
            (
                "なお、この確認で分かるのは「取り出せるかどうか」です。"
                "探す時間がどれだけ短くなるかは、実際の運用で計測しないと分かりません。本資料では推定値を示していません。",
    ...
    slide = _content_slide(
        prs,
        "必要な環境と、CADライセンスの考え方",
        "ナレッジシステム専用にCADを買い足すことは、必須ではありません",
    )
    _table(
        slide,
        0.5,
        1.8,
        12.33,
        ["構成", "読み取り方", "CADの要否", "取り出せる情報"],
        [
            [
                "構成A",
                "3D CAD（ICAD）の図面をそのまま読み取る",
                "登録する時だけ必要",
                "最も多い（材質・質量・部品構成まで）",
            ],
            [
    ...
    for index, (head, body) in enumerate(
        [
            (
                "CADが要るのは登録の時だけ",
                "検索・閲覧・見出しの表示にCADは要りません。読み取った内容は保存され、以降は保存された情報を見ます。",
            ),
            (
                "既にお持ちのCADを使えます",
                "設計者が使っていない時間帯に読み取りを動かす、といった運用が可能です。ご利用条件は事前に確認します。",
            ),
            (
                "CADなしでも始められます",
                "STEP／DXFに出力したデータを取り込む構成であれば、CADなしで運用できます。取り出せる情報は少なくなります。",
            ),
        ]
    ):
        x = 0.5 + index * 4.17
    ...
    _note(
        slide,
        6.3,
        "構成Aと構成Bは併用できます。ふだんは構成Bで運用し、詳しい情報が必要な図面だけ構成Aで読み取る、といった使い方も可能です。",
    )
    ...
    preparations = [
        ("使う言葉を登録する", "自社の客先名・装置名・案件名を登録します。ここに入っている言葉だけが見出しになります。運用しながら足していけます。"),
        ("フォルダの付け方をそろえる", "客先名や案件名がフォルダ名から読み取れると、精度が上がります。今後の新規案件だけでも規則をそろえる価値があります。"),
    ...
    _note(
        slide,
        6.5,
        "いずれも大がかりな整備は不要です。まず対象を絞って始め、運用しながら言葉と規則を足していく進め方を想定しています。",
    )
    ...
    _textbox(
        ...
        + [
            ("", {"size": 10, "space_after": 10}),
            (
                "効果を数値でお示しするには、実際の業務で試験運用し、導入前後を計測する必要があります。"
                "根拠のない数値は本資料に載せていません。",
    ...
    slide = _content_slide(
        prs,
        "導入の進め方",
        "小さく試して、効果を確かめてから広げる進め方をおすすめしています",
    )
    phases = [
        ("STEP 1", "小さく試す", ["対象の案件・装置を1つに絞る", "その範囲の図面を登録する", "どこまで取り出せるかを見る"]),
        ("STEP 2", "言葉と運用を整える", ["自社の客先名・装置名を登録する", "フォルダの付け方をそろえる", "登録と確認の担当を決める"]),
    ...
    # --- 16. 最終スライド ---------------------------------------------------
    prs.slides.add_slide(prs.slide_layouts[LAYOUT_CLOSING])
